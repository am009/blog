...
来自：[remote desktop - "Windows Defender Credential Guard does not allow using saved credentials" for RDP connections? - Super User](https://superuser.com/questions/1756354/windows-defender-credential-guard-does-not-allow-using-saved-credentials-for-r)

## 调试docker构建过程 docker image build debug dockerfile

首先，在docker file里需要调试的地方增加 `RUN sleep infinity` 然后使用`ps -aux | grep sleep`找到sleep的uid，并使用 `sudo nsenter -a -t $PID_OF_SLEEP sh` 进入构建过程调试。
