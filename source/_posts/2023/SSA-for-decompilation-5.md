...
（译者注：这里“高级语言表达的低级反汇编”说得很好。很多时候遇到人不懂lifter和decompiler产生的IR的区别，比如觉得都是转LLVM IR，转出来不就可以了。经常不知道怎么反驳。。。）
...
5.6 节展示了 Mycroft 约束的修改，它考虑了将常量添加到结构指针所导致的异常。为了在基于数据流的分析中表示 Mycroft 的约束，需要一些额外的类型和运算符。让 π 在格子中为指针或整数或更高。希望所有出现的 π 最终都将被替换为更低（更精确）的类型，例如整数或特定的指针类型。整数被理解为与原始程序体系结构中的指针大小相同。在图 5.12 的格子中，π 的值可以是指针或整数、size32 或 >。与 add a, b, c 相关的数据流方程（即 c=a+b），结构指针例外，可以重述为：
$$
\begin{align}
...
\end{align}
$$
其中 Σa 和 Σs（a 代表加数或被加数，s 代表总和）是特殊函数，定义如下

![image-20230518163304404](image-20230518163304404.png)
