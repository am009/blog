...
[^1]: OCaml 在这些打破原则的特性上一直相对保守，显著的例外是 GADTs 和多重结构体字段，但 Haskell 在这方面的采用更为宽松，以增强表达能力。  
...
[^7]: ¹²在 Scala 中，`opt.fold(t)(f)` 通过对包含的值调用函数 `f` 来折叠 `opt` 选项，如果选项为空，则评估默认的 `thunk t`。 `MutMap` 的 `map.getOrElseUpdate(k, t)` 方法在 `map` 中查找一个键 `k`；如果未找到该键，则评估 `thunk t` 并用该值更新 `map`；否则，返回与该键关联的值。
